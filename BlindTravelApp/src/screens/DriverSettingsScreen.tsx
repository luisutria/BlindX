...
export default function DriverSettingsScreen({ navigation, route }: any) {
  // Recibimos el ID del conductor
  ...
  // NUEVO ESTADO: Preferencia de Mapa
  ...
  // Función para alternar entre Waze y Google Maps
  const toggleMapPreference = () => {
    const newMap = mapPreference === 'Google Maps' ? 'Waze' : 'Google Maps';
    setMapPreference(newMap);
    Alert.alert("Preferencia Actualizada", `Ahora usaremos ${newMap} para tus rutas.`);
  };

  const handleDeleteAccount = () => {
    if (!userId) {
        Alert.alert("Error", "No se pudo identificar tu cuenta. Cierra sesión e intenta de nuevo.");
        return;
    }
    Alert.alert(
      "¿Eliminar cuenta de Conductor?",
      "Perderás tu historial de viajes y ganancias acumuladas.",
      [
  ...
  const confirmDelete = async () => {
    ...
    try {
        ...
        if (response.ok) {
            Alert.alert("Cuenta Eliminada", "Lamentamos que te vayas.");
            navigation.popToTop();
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Configuración Conductor</Text>
        <View style={{ width: 28 }} /> 
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <TouchableOpacity 
            ...
            onPress={toggleMapPreference}
        >
            <View style={styles.rowLeft}>
                <View style={styles.iconBlueWrapper}>
                    <Ionicons name="navigate-circle-outline" size={24} color="#2563EB" />
                </View>
                <View>
                    <Text style={styles.settingText}>App de Mapa Predeterminada</Text>
                    <Text style={styles.subText}>Toca para cambiar</Text>
                </View>
            </View>
            <View style={styles.rowRight}>
                 <Text style={[styles.valueText, { color: '#2563EB', fontWeight: 'bold' }]}>
                    {mapPreference}
                 </Text>
      ...
      <Modal animationType="fade" transparent={true} visible={showHelpModal} onRequestClose={() => setShowHelpModal(false)}>
        <View style={styles.modalOverlay}>
            <View style={styles.helpModalContent}>
                ...
                <Text style={styles.helpBody}>Como conductor, presiona 4 veces rápido la pantalla si te sientes en peligro.</Text>
...
const styles = StyleSheet.create({
  ...
  // Nuevo estilo para el ícono de mapa
  iconBlueWrapper: { backgroundColor: '#DBEAFE', padding: 8, borderRadius: 8 },
  subText: { fontSize: 12, color: '#666', marginLeft: 12, marginTop: 2 },
